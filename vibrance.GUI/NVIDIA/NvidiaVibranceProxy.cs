...
namespace vibrance.GUI
{
    ...
    public class NvidiaVibranceProxy : IVibranceProxy
    {
        ...
        public const string NVAPI_ERROR_GET_MONITOR_HANDLE =
            "Couldn't determine the monitor handle CSGO is running in. :( Using main output!";

        public const string NVAPI_WARNING_MULTIPLE_MONITORS =
            "You are using multiple monitors. Start CSGO, Tab out and click ok after. Do this once then keep vibranceGUI open (means you can start and quit CSGO several times).";
        ...
        public NvidiaVibranceProxy(bool isSilenced = false)
        {
            try
            {
                ...
                var ret = initializeLibrary();

                var gpuHandles = new int[NVAPI_MAX_PHYSICAL_GPUS];
                var outputIds = new int[NVAPI_MAX_PHYSICAL_GPUS];
        ...
        public void handleDVC()
        {
            ...
            while (vibranceInfo.shouldRun)
            {
                ...
                if (isCsgoStarted(ref hwnd) && hwnd != IntPtr.Zero)
                {
                    ...
                    {
                        var nLen = GetWindowTextLength(hwnd);
                        if (nLen > 0)
                        {
                            ...
                            if (sb != null && sb.ToString().Equals(NVAPI_GLOBAL_OFFENSIVE_WINDOW_NAME))
                            {
                                ...
                                {
                                    var csgoHandle = getCsgoDisplayHandle();
                                    if (csgoHandle != -1)
                                    {
                ...
                {
                    if (vibranceInfo.affectPrimaryMonitorOnly &&
                        !equalsDVCLevel(vibranceInfo.defaultHandle, vibranceInfo.userVibranceSettingDefault))
                    {
                        setDVCLevel(vibranceInfo.defaultHandle, vibranceInfo.userVibranceSettingDefault);
                        isChanged = false;
                    }
                    else if (!vibranceInfo.affectPrimaryMonitorOnly &&
                             !vibranceInfo.displayHandles.TrueForAll(
                                 handle => equalsDVCLevel(handle, vibranceInfo.userVibranceSettingDefault)))
                    {
                        vibranceInfo.displayHandles.ForEach(
                            handle => setDVCLevel(handle, vibranceInfo.userVibranceSettingDefault));
                        isChanged = false;
                ...
                Thread.Sleep(vibranceInfo.sleepInterval);
            }
            handleDVCExit();
        }
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Ansi)]
        private static extern bool getDVCInfo(ref NV_DISPLAY_DVC_INFO info, int defaultHandle);
        ...
        [DllImport(
            ...
            CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto)]
        private static extern bool isCsgoActive(ref IntPtr hwnd);
